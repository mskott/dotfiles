...
- [[#custom-functions][Custom functions]]
  - [[#open-customer-notes][Open customer notes]]

* use-package
~use-package~ is configured in [[file:init.el][init.el]]. Here is [[https://ianyepan.github.io/posts/setting-up-use-package/][a simple tutorial]] and [[https://batsov.com/articles/2025/04/17/using-use-package-the-right-way/][some hints]] about configuring packages in an optimised way.

[[info:use-package#Top][The manual]] is shipped with Emacs.
...
* EMACS
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (windmove-default-keybindings 'meta) ; Enable windmove with meta as modifier
  #+END_SRC
* Emacs server
Start the Emacs server when Emacs starts.
...
* Fonts
  ...
  Fonts are strange - be carefull when setting height, 20 is way too small.
...
** General Org configuration
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      (org-agenda-files (list "~/Org" "~/Org/Customers" "~/Org/Products"))
      (org-refile-targets '((org-agenda-files . (:maxlevel . 2))))
      (org-special-ctrl-a/e t)           ; Change C-a/C-e behaviour on headlines
      (org-startup-with-inline-images t) ; Display inline images on startup
      (org-startup-folded 'content)      ; Fold contents when opening file
...
In-buffer completion using [[https://github.com/minad/corfu][Corfu]]. In the same family as many of the tools for [[*Minibuffer and completion][mini-buffer completion]].

[[https://kristofferbalintona.me/posts/202202270056/][Article with nice samples of configuration.]]

#+begin_src emacs-lisp
  (use-package corfu
    :init
    (global-corfu-mode)
    (corfu-popupinfo-mode)            ; Display documentation in popup
    :custom
    (corfu-auto t)                    ; Enable auto completion
    (corfu-quit-no-match 'separator)) ; Quit if no candidate found
#+end_src
...
The language server for YAML also isn't packaged. Can be installed using ~npm install yaml-language-server~ and also adding it to the PATH.
...
* Custom functions
** Open customer notes
Open notes about specific customer with completion using ~C-c f~
#+begin_src emacs-lisp
  (defvar my-customer-notes-path "~/Org/Customers")
  (defun my-find-customer-notes ()
    "Open notes about a customer"
    (interactive)
      (find-file (expand-file-name
                  (completing-read "Customer: " (directory-files my-customer-notes-path nil "\\.org$"))
                  my-customer-notes-path)))
  (global-set-key (kbd "C-c f") 'my-find-customer-notes)
#+end_src
